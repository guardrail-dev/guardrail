      ...
      val companion = q"""
        object Pet {
          implicit val encodePet: Encoder.AsObject[Pet] = {
            ...
            new Encoder.AsObject[Pet] { final def encodeObject(a: Pet): JsonObject = JsonObject.fromIterable(Vector(("name", a.name.asJson), ("underscore_name", a.underscoreName.asJson), ("dash-name", a.dashName.asJson))) }.mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
          }
