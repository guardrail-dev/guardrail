    ...
    val companion = q"""
      object Foo {
        implicit val encodeFoo: Encoder.AsObject[Foo] = {
          ...
          new Encoder.AsObject[Foo] { final def encodeObject(a: Foo): JsonObject = JsonObject.fromIterable(Vector(("nonEmptyString", a.nonEmptyString.asJson), ("positiveLong", a.positiveLong.asJson))) }.mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
        }
