    ...
    val companion  = q"""
      object `dashy-class` {
        implicit val `encodedashy-class`: Encoder.AsObject[`dashy-class`] = {
          ...
          new Encoder.AsObject[`dashy-class`] { final def encodeObject(a: `dashy-class`): JsonObject = JsonObject.fromIterable(Vector(("dashy-param", a.dashyParam.asJson))) }.mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
        }
