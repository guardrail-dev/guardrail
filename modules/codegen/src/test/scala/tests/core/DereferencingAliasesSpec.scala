    ...
    val companion = q"""
      object propRef {
        implicit val encodepropRef: Encoder.AsObject[propRef] = {
          ...
          new Encoder.AsObject[propRef] { final def encodeObject(a: propRef): JsonObject = JsonObject.fromIterable(Vector(("param", a.param.asJson), ("array", a.array.asJson), ("arrayArray", a.arrayArray.asJson))) }.mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
        }
