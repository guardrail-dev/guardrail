    ...
    val companion  = q"""
      object Foo {
        implicit val encodeFoo: _root_.io.circe.Encoder.AsObject[Foo] = {
          ...
          _root_.io.circe.Encoder.AsObject.instance[Foo](a => _root_.io.circe.JsonObject.fromIterable(Vector(("conflicting_name", a.conflicting_name.asJson), ("ConflictingName", a.ConflictingName.asJson)))).mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
        }
