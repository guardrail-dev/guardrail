    ...
    val companion  = q"""
      object Foo {
        implicit val encodeFoo: Encoder.AsObject[Foo] = {
          ...
          new Encoder.AsObject[Foo] { final def encodeObject(a: Foo): JsonObject = JsonObject.fromIterable(Vector(("conflicting_name", a.conflicting_name.asJson), ("ConflictingName", a.ConflictingName.asJson))) }.mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
        }
