    ...
    val companion  = q"""
      object First {
        implicit val encodeFirst: Encoder.AsObject[First] = {
          ...
          new Encoder.AsObject[First] { final def encodeObject(a: First): JsonObject = JsonObject.fromIterable(Vector(("a", a.a.asJson))) }.mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
        }
    ...
    val companion  = q"""
      object Fifth {
        implicit val encodeFifth: Encoder.AsObject[Fifth] = {
          val readOnlyKeys = Set[String]()
          new Encoder.AsObject[Fifth] { final def encodeObject(a: Fifth): JsonObject = JsonObject.fromIterable(Vector(("a_b_c_d", a.aBCD.asJson), ("b_c_d_e", a.bCDE.asJson))) }.mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
        }
    ...
    val companion  = q"""
      object Sixth {
        implicit val encodeSixth: Encoder.AsObject[Sixth] = {
          val readOnlyKeys = Set[String]()
          new Encoder.AsObject[Sixth] { final def encodeObject(a: Sixth): JsonObject = JsonObject.fromIterable(Vector(("defval", a.defval.asJson), ("defval_opt", a.defvalOpt.asJson))) }.mapJsonObject(_.filterKeys(key => !(readOnlyKeys contains key)))
        }
