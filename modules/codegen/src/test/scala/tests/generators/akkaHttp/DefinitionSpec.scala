...
class DefinitionSpec extends FunSuite with Matchers with SwaggerSpecRunner {
  ...
  test("Plain objects should be generated") {
    val (
      ProtocolDefinitions(ClassDefinition(_, _, _, _, cls, staticDefns, _) :: _, _, _, _),
      _,
      _
  ...
  test("Camel case conversion should happen") {
    val (
      ProtocolDefinitions(_ :: _ :: _ :: _ :: ClassDefinition(_, _, _, _, cls, staticDefns, _) :: _, _, _, _),
      _,
      _
    )       = runSwaggerSpec(swagger)(Context.empty, AkkaHttp)
    val cmp = companionForStaticDefns(staticDefns)
  ...
  test("Defaults should work") {
    val (
      ProtocolDefinitions(_ :: _ :: _ :: _ :: _ :: ClassDefinition(_, _, _, _, cls, staticDefns, _) :: _, _, _, _),
      _,
      _
    )       = runSwaggerSpec(swagger)(Context.empty, AkkaHttp)
    val cmp = companionForStaticDefns(staticDefns)
