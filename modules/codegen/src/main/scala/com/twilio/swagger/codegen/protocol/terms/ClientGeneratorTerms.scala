...
class ClientTerms[F[_]](implicit I: Inject[ClientTerm, F]) {
  ...
  def getImports(tracing: Boolean): Free[F, Seq[Import]] =
  ...
  def clientClsArgs(tracingName: Option[String], schemes: Seq[String], host: Option[String], tracing: Boolean): Free[F, Seq[Seq[Term.Param]]] =
    Free.inject[ClientTerm, F](ClientClsArgs(tracingName, schemes, host, tracing))
  def buildCompanion(clientName: String, tracingName: Option[String], schemes: Seq[String], host: Option[String], ctorArgs: Seq[Seq[Term.Param]], tracing: Boolean): Free[F, Defn.Object] =
    Free.inject[ClientTerm, F](BuildCompanion(clientName, tracingName, schemes, host, ctorArgs, tracing))
  def buildClient(clientName: String, tracingName: Option[String], schemes: Seq[String], host: Option[String], basePath: Option[String], ctorArgs: Seq[Seq[Term.Param]], clientCalls: Seq[Defn], tracing: Boolean): Free[F, Defn.Class] =
