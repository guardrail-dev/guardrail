...
import dev.guardrail.core.extract.{ CustomArrayTypeName, CustomMapTypeName, CustomTypeName, DataRedaction, EmptyValueIsNull }
...
object CirceRefinedProtocolGenerator {
  def apply(circeRefinedVersion: CirceRefinedModelGenerator): ProtocolTerms[ScalaLanguage, Target] =
    new CirceRefinedProtocolGenerator(circeRefinedVersion.toCirce, applyValidations)
