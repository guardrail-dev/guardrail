      ...
      extends PathExtractor[ScalaLanguage, Term, Term.Name, ModelGeneratorType](
        pathSegmentConverter = {
          case (LanguageParameter(_, param, _, argName, argType), base, modelGeneratorType) =>
            import dev.guardrail.generators.Scala.AkkaHttpHelper.fromStringConverter
            base.fold {
              argType match {
  ...
  class ServerTermInterp(modelGeneratorType: ModelGeneratorType)(implicit Cl: CollectionsLibTerms[ScalaLanguage, Target])
    ...
    def pathStrToAkka(
      ...
      ((basePath.getOrElse("") + path.unwrapTracker).stripPrefix("/") match {
        ...
        case fullPath =>
          paths.generateUrlPathExtractors(Tracker.cloneHistory(path, fullPath), pathArgs, modelGeneratorType)
      })
    }
