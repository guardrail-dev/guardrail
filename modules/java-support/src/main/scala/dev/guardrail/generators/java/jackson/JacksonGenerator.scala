...
class JacksonGenerator private (implicit Cl: CollectionsLibTerms[JavaLanguage, Target], Ca: CollectionsAbstraction[JavaLanguage])
  ...
  override def renderMembers(
      clsName: String,
      elems: RenderedEnum[JavaLanguage]
  ) =
    Target.pure(None)

  override def encodeEnum(clsName: String, tpe: com.github.javaparser.ast.`type`.Type): Target[Option[BodyDeclaration[_ <: BodyDeclaration[_]]]] =
    Target.pure(None)

  override def decodeEnum(clsName: String, tpe: com.github.javaparser.ast.`type`.Type): Target[Option[BodyDeclaration[_ <: BodyDeclaration[_]]]] =
    Target.pure(None)
  ...
  override def extractConcreteTypes(definitions: Either[String, List[PropMeta[JavaLanguage]]]) =
