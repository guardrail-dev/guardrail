package tests.generators.zio.http
...
class ZioHttpServerTest extends AnyFunSuite with Matchers with SwaggerSpecRunner {
  ...
  test(s"Ensure routes are generated") {
    val (
      _,
      _,
      Servers(Server(_, _, genHandler, genResource :: _) :: Nil, Nil)
    ) = runSwaggerSpec(scalaInterpreter)(swagger)(Context.empty, ZioHttpVersion.value)
  ...
  test(s"Ensure routes are generated with tracing") {
    val (
      _,
      _,
      Servers(Server(_, _, genHandler, genResource :: _) :: Nil, Nil)
    ) = runSwaggerSpec(scalaInterpreter)(swagger)(Context.empty.copy(tracing = true), ZioHttpVersion.value)
  ...
  test(s"Ensure routes are generated with custom extraction") {
    val (
      _,
      _,
      Servers(Server(_, _, genHandler, genResource :: _) :: Nil, Nil)
    ) = runSwaggerSpec(scalaInterpreter)(swagger)(Context.empty.copy(customExtraction = true), ZioHttpVersion.value)
    val handler =
      q"""
