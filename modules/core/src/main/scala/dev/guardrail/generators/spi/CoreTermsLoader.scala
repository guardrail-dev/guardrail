...
import dev.guardrail.{ Args, Common, MissingDependency, ReadSpec, Target, WriteTree }
...
trait CoreTermsLoader {
  ...
  def apply(language: String, parameters: NonEmptyList[Args]): Option[Function0[Target[NonEmptyList[ReadSpec[Target[List[WriteTree]]]]]]]
}
...
object CoreTermsLoader {
  ...
  @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
  def load(language: String, parameters: NonEmptyList[Args], error: MissingDependency): Target[NonEmptyList[ReadSpec[Target[List[WriteTree]]]]] = {
    val found = loader
      .iterator()
      .asScala
